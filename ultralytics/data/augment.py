...
class Mosaic(BaseMixTransform):
    ...
    def _cat_labels(self, mosaic_labels: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for labels in mosaic_labels:
            ...
            if "orientation" in labels:
                orientation.append(labels["orientation"])
        ...
        if orientation:
            final_labels["orientation"] = np.concatenate(orientation, 0)
        final_labels["instances"].clip(imgsz, imgsz)
        good = final_labels["instances"].remove_zero_area_boxes()
        final_labels["cls"] = final_labels["cls"][good]
        if "completeness" in final_labels:
            final_labels["completeness"] = final_labels["completeness"][good]
        if "orientation" in final_labels:
            final_labels["orientation"] = final_labels["orientation"][good]
...
class CutMix(BaseMixTransform):
    ...
    def _mix_transform(self, labels: dict[str, Any]) -> dict[str, Any]:
        ...
        if "completeness" in labels:
            labels["completeness"] = np.concatenate([labels["completeness"], labels2["completeness"][indexes2]], axis=0)
        if "orientation" in labels:
            labels["orientation"] = np.concatenate([labels["orientation"], labels2["orientation"][indexes2]], axis=0)
...
class RandomPerspective:
    ...
    def __call__(self, labels: dict[str, Any]) -> dict[str, Any]:
        ...
        labels["cls"] = cls[i]
        if "completeness" in labels:
            labels["completeness"] = labels["completeness"][i]
        if "orientation" in labels:
            labels["orientation"] = labels["orientation"][i]
        labels["img"] = img
...
class Albumentations:
    ...
    def __init__(self, p: float = 1.0, transforms: list | None = None) -> None:
        ...
        try:
            ...
            self.label_fields = ["class_labels", "completeness_labels", "orientation_labels"]
            self.transform = (
    ...
    def __call__(self, labels: dict[str, Any]) -> dict[str, Any]:
        ...
        if self.contains_spatial:
            ...
            if len(cls):
                ...
                completeness_labels = completeness if completeness is not None else np.zeros_like(cls)
                orientation_labels = orientation if orientation is not None else np.zeros_like(cls)
                new = self.transform(
                    image=im,
                    bboxes=bboxes,
                    class_labels=cls,
                ...
                if len(new["class_labels"]) > 0:  # skip update if no bbox in new im
                    ...
                    if completeness is not None:
                        labels["completeness"] = np.array(new["completeness_labels"]).reshape(-1, 1)
                    if orientation is not None:
                        labels["orientation"] = np.array(new["orientation_labels"]).reshape(-1, 1)
...
class Format:
    ...
    def __call__(self, labels: dict[str, Any]) -> dict[str, Any]:
        ...
        if completeness is not None:
            labels["completeness"] = torch.from_numpy(completeness) if nl else torch.zeros(nl, 1)
        if orientation is not None:
            labels["orientation"] = torch.from_numpy(orientation) if nl else torch.zeros(nl, 1)
...
class RandomLoadText:
    ...
    def __call__(self, labels: dict[str, Any]) -> dict[str, Any]:
        ...
        if "completeness" in labels:
            labels["completeness"] = labels["completeness"][valid_idx]
        if "orientation" in labels:
            labels["orientation"] = labels["orientation"][valid_idx]
